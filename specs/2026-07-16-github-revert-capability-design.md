...
```
1. GET  /repos/{o}/{r}/git/commits/{mergeSha}     → merge commit details
2. GET  /repos/{o}/{r}/git/commits/{parentSha}     → first parent's tree SHA
3. POST /repos/{o}/{r}/git/commits                 → revert commit (tree=parent's tree, parent=mergeSha)
...
6. POST /repos/{o}/{r}/pulls                       → revert PR (temp → target)
7. PUT  /repos/{o}/{r}/pulls/{n}/merge             → merge the revert PR
8. DELETE /repos/{o}/{r}/git/refs/heads/revert/...  → cleanup temp branch
```

Steps 4+5 use the delete-before-create idempotency pattern from
`GitHubBatchBranchClient`. Step 7 uses `merge_method: "merge"` (not squash) to
...
public interface RevertClient {
    RevertOutcome revert(String owner, String repo, String targetBranch, String mergeSha);
}
```
...
public sealed interface RevertOutcome {
    ...
    record MergeConflict(String reason) implements RevertOutcome {}
...
- `MergeConflict` — revert PR cannot merge because subsequent commits on the
  target branch conflict with the revert. The rollback worker escalates to human
  oversight (`HumanOversight.ROUTING_REVIEW`).
...
Three new endpoints on the existing `@RegisterRestClient(configKey = "github-api")`
interface:
...
@PATCH
@Path("/{owner}/{repo}/git/refs/heads/{ref}")
GitRef updateRef(@PathParam("owner") String owner,
                 @PathParam("repo") String repo,
...
The `updateRef` endpoint is added for completeness — it provides atomic ref
updates vs the current delete+create pattern. The revert flow itself uses the
temp-branch+PR approach so doesn't need `updateRef`, but it's a natural addition
to `GitHubGitApi` and may be useful for future operations.

## GitHubPullRequestApi

New REST client interface for PR creation:
...
Returns raw `Map<String, Object>` matching the existing convention.
...
1. **Get merge commit** — `gitApi.getCommit(owner, repo, mergeSha)`. Validate it
   has at least one parent (not a root commit).
...
| 1 | Merge commit has no parents | `Failure("not a merge commit")` |
| 4–5 | API error | `Failure` (temp branch cleanup attempted) |
| 6 | 409 Conflict | `MergeConflict` (temp branch cleanup attempted) |
| 6 | Other API error | `Failure` (temp branch cleanup attempted) |
| 7 | Any error | Swallowed — logged, not surfaced |

Steps 4–7 are wrapped so that temp branch cleanup always runs regardless of
...
| `github/` | `GitHubGitApi.java` | Modified — 3 new methods |
...
| | | — Merge conflict: step 6 returns 409 → `MergeConflict`, temp branch cleaned up |
| | | — API error on getCommit (step 1) → `Failure` |
| | | — API error on createCommit (step 3) → `Failure` |
| | | — API error on createPullRequest (step 5) → `Failure`, temp branch cleaned up |
...
| | | — No-parent commit: getCommit returns empty parents → `Failure("not a merge commit")` |
...
```java
RevertOutcome outcome = revertClient.revert(owner, repo, targetBranch, mergeSha);
switch (outcome) {
    case Success s    -> // record revert in EventLog, continue
    case MergeConflict mc -> // escalate to HumanOversight.ROUTING_REVIEW
